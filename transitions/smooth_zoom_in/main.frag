...
void main() {
    ...
    // Scale curves:
    // Outgoing clip A accelerates into high zoom from 1.0 up to zoomIntensity
    float tA = clamp(p * 2.0, 0.0, 1.0);
    float scaleA = mix(1.0, zoomIntensity, tA * tA * (3.0 - 2.0 * tA));

    // Incoming clip B rushes in from smaller scale / high speed towards 1.0
    float tB = clamp((p - 0.45) / 0.55, 0.0, 1.0);
    float scaleB = mix(1.0 / zoomIntensity, 1.0, sqrt(tB));
